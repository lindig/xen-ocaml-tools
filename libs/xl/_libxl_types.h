...
static int domain_config_val (libxl_ctx *ctx, libxl_domain_config *c_val, value v)
{
	...
	{
		int i;
		c_val->num_vdispls = Wosize_val(Field(v, 11));
		c_val->vdispls = (libxl_device_vdispl *) calloc(c_val->num_vdispls, sizeof(*c_val->vdispls));
		for(i=0; i<c_val->num_vdispls; i++) {
			device_vdispl_val(ctx, &c_val->vdispls[i], Field(Field(v, 11), i));
		}
	}
	
	{
		int i;
		c_val->num_channels = Wosize_val(Field(v, 12));
		c_val->channels = (libxl_device_channel *) calloc(c_val->num_channels, sizeof(*c_val->channels));
		for(i=0; i<c_val->num_channels; i++) {
			device_channel_val(ctx, &c_val->channels[i], Field(Field(v, 12), i));
		}
	}
	
	{
		int i;
		c_val->num_usbctrls = Wosize_val(Field(v, 13));
		c_val->usbctrls = (libxl_device_usbctrl *) calloc(c_val->num_usbctrls, sizeof(*c_val->usbctrls));
		for(i=0; i<c_val->num_usbctrls; i++) {
			device_usbctrl_val(ctx, &c_val->usbctrls[i], Field(Field(v, 13), i));
		}
	}
	
	{
		int i;
		c_val->num_usbdevs = Wosize_val(Field(v, 14));
		c_val->usbdevs = (libxl_device_usbdev *) calloc(c_val->num_usbdevs, sizeof(*c_val->usbdevs));
		for(i=0; i<c_val->num_usbdevs; i++) {
			device_usbdev_val(ctx, &c_val->usbdevs[i], Field(Field(v, 14), i));
		}
	}
	
	action_on_shutdown_val(ctx, &c_val->on_poweroff, Field(v, 15));
	action_on_shutdown_val(ctx, &c_val->on_reboot, Field(v, 16));
	action_on_shutdown_val(ctx, &c_val->on_watchdog, Field(v, 17));
	action_on_shutdown_val(ctx, &c_val->on_crash, Field(v, 18));
	action_on_shutdown_val(ctx, &c_val->on_soft_reset, Field(v, 19));
...
static value Val_domain_config (libxl_domain_config *domain_config_c)
{
	...
	CAMLlocal1(domain_config_ocaml);
	{
		...
		domain_config_ocaml = caml_alloc_tuple(20);
