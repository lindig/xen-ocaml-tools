...
let log_rotate ref_ch log_file log_nb_files =
  ...
  let log_files =
    let rec aux accu n =
      ...
      else if n = 1 && Sys.file_exists log_file then aux [(log_file, 1)] 2
      else
        let file = file (n - 1) in
        if Sys.file_exists file then aux ((file, n) :: accu) (n + 1) else accu
    in
    aux [] 1
  in
...
let make_file_logger log_file log_nb_files log_nb_lines log_nb_chars
    post_rotate =
...
let xenstored_log_destination =
  ref (File (Paths.xen_log_dir ^ "/xenstored.log"))
...
let init_xenstored_log () =
  ...
  | Syslog facility -> set_xenstored_logger (make_syslog_logger facility)
...
let init_access_log post_rotate =
  ...
  | Syslog facility -> access_logger := Some (make_syslog_logger facility)
  ...
  try
    maybe
      (fun logger ->
        ...
        let msg =
          Printf.sprintf "%s %s %s %s" prefix tid access_type censored_data
        in
...
let xb_op ~tid ~con ~ty data =
  let print =
    ...
    | Xenbus.Xb.Op.Introduce | Xenbus.Xb.Op.Release
     |Xenbus.Xb.Op.Getdomainpath | Xenbus.Xb.Op.Isintroduced
     |Xenbus.Xb.Op.Resume ->
