...
module Node = struct
  type t =
    {name: Symbol.t; perms: Perms.Node.t; value: string; children: t list}
 ...
 *)
  ...
  let rec lookup_modify node path fct =
    ...
    | h :: l ->
        let n, c =
          if not (Node.exists node h) then raise (Define.Lookup_Doesnt_exist h)
  ...
  let get_node rnode path =
    if path = [] then Some rnode
    else try Some (lookup_get rnode path) with Define.Doesnt_exist -> None
  ...
  let rec get_deepest_existing_node node = function
    | [] -> (node, true)
    | h :: t -> (
      try get_deepest_existing_node (Node.find node h) t with Not_found ->
        (node, false) )
...
let ls store perm path =
  let children =
    if path = [] then Node.get_children store.root
    else
      let do_ls node name =
...
let getperms store perm path =
  if path = [] then Node.get_perms store.root
  else
    let fct n name =
...
let write store perm path value =
  ...
  store.root <- root ;
  if node_created then Quota.add_entry store.quota owner
...
let rm store perm path =
  ...
  | Some rmed_node ->
      ...
      Node.recurse
        (fun node -> Quota.del_entry store.quota (Node.get_owner node))
        rmed_node
...
let stats store =
  ...
  traversal store.root (fun path node -> incr nb_nodes) ;
