...
let process_watch ops cons =
  let do_op_watch op cons =
    let recurse =
      ...
      | _ -> raise (Failure "huh ?")
    in
    Connections.fire_watches cons (snd op) recurse
  in
...
let create_implicit_path t perm path =
  ...
  if not (Transaction.path_exists t dirname) then
    let rec check_path p =
      match p with
      | [] -> []
      | h :: l -> if Transaction.path_exists t h then check_path l else p
    in
...
let do_debug con t domains cons data =
  if (not (Connection.is_dom0 con)) && not !allow_debug then None
  else
    try
...
let do_directory con t domains cons data =
  ...
  if List.length entries > 0 then Utils.join_by_null entries ^ "\000" else ""
...
let do_error con t domains cons data = raise Define.Unknown_operation
...
let send_response ty con t rid response =
  ...
  | Packet.Error e -> Connection.send_error con (Transaction.get_id t) rid e
...
let function_of_type_simple_op ty =
  match ty with
  | Xenbus.Xb.Op.Debug | Xenbus.Xb.Op.Watch | Xenbus.Xb.Op.Unwatch
   |Xenbus.Xb.Op.Transaction_start | Xenbus.Xb.Op.Transaction_end
   |Xenbus.Xb.Op.Introduce | Xenbus.Xb.Op.Release | Xenbus.Xb.Op.Isintroduced
   |Xenbus.Xb.Op.Resume | Xenbus.Xb.Op.Set_target
   |Xenbus.Xb.Op.Reset_watches | Xenbus.Xb.Op.Invalid ->
...
let input_handle_error ~cons ~doms ~fct ~con ~t ~req =
  ...
  try fct con t doms cons req.Packet.data with
        ...
        (fun () ->
          ...
          | Transaction_again ->
              ...
              if Hashtbl.length guilty_cons = 0 then (
                debug
                  "Found no culprit for conflict in %s: must be self or not \
                   in history."
                  con ;
...
let do_transaction_start con t domains cons data =
  if Transaction.get_id t <> Transaction.none then raise Transaction_nested ;
...
let do_transaction_end con t domains cons data =
  ...
  in
  if not success then raise Transaction_again ;
  if commit then (
    process_watch (List.rev (Transaction.get_paths t)) cons ;
    match t.Transaction.ty with
    | Transaction.No -> () (* no need to record anything *)
    | Transaction.Full (id, oldstore, cstore) ->
...
let do_introduce con t domains cons data =
  ...
  in
  let dom =
    if Domains.exist domains domid then Domains.find domains domid
    else
      try
...
let do_release con t domains cons data =
  ...
  if fire_spec_watches then Connections.fire_spec_watches cons "@releaseDomain"
...
let do_resume con t domains cons data =
  ...
  let domid =
    ...
    | _ -> raise Invalid_Cmd_Args
  in
  if Domains.exist domains domid then Domains.resume domains domid
  else raise Invalid_Cmd_Args
...
let retain_op_in_history ty =
  match ty with
  | Xenbus.Xb.Op.Write | Xenbus.Xb.Op.Mkdir | Xenbus.Xb.Op.Rm
   |Xenbus.Xb.Op.Setperms ->
      true
  | Xenbus.Xb.Op.Debug | Xenbus.Xb.Op.Directory | Xenbus.Xb.Op.Read
   |Xenbus.Xb.Op.Getperms | Xenbus.Xb.Op.Watch | Xenbus.Xb.Op.Unwatch
   |Xenbus.Xb.Op.Transaction_start | Xenbus.Xb.Op.Transaction_end
   |Xenbus.Xb.Op.Introduce | Xenbus.Xb.Op.Release
   |Xenbus.Xb.Op.Getdomainpath | Xenbus.Xb.Op.Watchevent | Xenbus.Xb.Op.Error
   |Xenbus.Xb.Op.Isintroduced | Xenbus.Xb.Op.Resume | Xenbus.Xb.Op.Set_target
   |Xenbus.Xb.Op.Reset_watches | Xenbus.Xb.Op.Invalid ->
      false
...
let process_packet ~store ~cons ~doms ~con ~req =
  ...
  try
    ...
    let t =
      ...
      else Connection.get_transaction con tid
    in
    ...
    let response =
      try
        if tid <> Transaction.none then
          ...
          Transaction.add_operation ~perm:(Connection.get_perm con) t req
            response ;
        response
...
let do_input store cons doms con =
  let newpacket =
    try Connection.do_input con with
    | Xenbus.Xb.Reconnect ->
    ...
    | Failure exp ->
        ...
        error "got a bad client %s"
          (sprintf "%-8s" (Connection.get_domstr con)) ;
...
let do_output store cons doms con =
  if Connection.has_output con then (
    ...
    try ignore (Connection.do_output con) with Xenbus.Xb.Reconnect ->
      info "%s requests a reconnect" (Connection.get_domstr con) ;
      Connection.reconnect con ;
      info "%s reconnection complete" (Connection.get_domstr con) )
