...
let add_domain cons dom =
  let xbcon =
    Xenbus.Xb.open_mmap (Domain.get_interface dom) (fun () -> Domain.notify dom)
  in
...
let iter_domains cons fct = Hashtbl.iter (fun k c -> fct c) cons.domains
...
let has_more_work cons =
  Hashtbl.fold
    (fun id con acc -> if Connection.has_more_work con then con :: acc else acc)
    cons.domains []
...
let add_watch cons con path token =
  ...
  let watches =
    if Trie.mem cons.watches key then Trie.find cons.watches key else []
  in
...
let del_watch cons con path token =
  ...
  if watches = [] then cons.watches <- Trie.unset cons.watches key
  else cons.watches <- Trie.set cons.watches key watches ;
  watch
...
let fire_watches cons path recurse =
  ...
  if recurse then Trie.iter fire_rec (Trie.sub cons.watches key)

let fire_spec_watches cons specpath =
  iter cons (fun con ->
      List.iter
        (fun w -> Connection.fire_single_watch w)
