...
(**************** high level binding ****************)
type t = {handle: Xeneventchn.handle; mutable virq_port: Xeneventchn.t option}

let init () = {handle= Xeneventchn.init (); virq_port= None}

let fd eventchn = Xeneventchn.fd eventchn.handle

let bind_dom_exc_virq eventchn =
  eventchn.virq_port <- Some (Xeneventchn.bind_dom_exc_virq eventchn.handle)
