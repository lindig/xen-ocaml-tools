...
type watch =
  {con: t; token: string; path: string; base: string; is_relative: bool}
...
let get_watch_path con path =
  if path.[0] = '@' || path.[0] = '/' then path
  else
    let rpath = get_path con in
    rpath ^ path

let get_watches (con : t) path =
  if Hashtbl.mem con.watches path then Hashtbl.find con.watches path else []
...
let add_watch con path token =
  if
    !Quota.activate && !Define.maxwatch > 0
    && (not (is_dom0 con))
  ...
  if List.exists (fun w -> w.token = token) l then raise Define.Already_exist ;
...
let del_watch con path token =
  ...
  if List.length filtered > 0 then Hashtbl.replace con.watches apath filtered
  else Hashtbl.remove con.watches apath ;
  con.nb_watches <- con.nb_watches - 1 ;
  (apath, w)
...
let del_transactions con = Hashtbl.clear con.transactions
...
let start_transaction con store =
  ...
  then raise Quota.Transaction_opened ;
