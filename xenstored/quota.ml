...
type t =
  { maxent: int
  ; (* max entities per domU *)
    maxsize: int
  ; (* max size of data store in one node *)
    cur: (Xenctrl.domid, int) Hashtbl.t
  (* current domains quota *) }

let to_string quota domid =
  if Hashtbl.mem quota.cur domid then
    Printf.sprintf "dom%i quota: %i/%i" domid
      (Hashtbl.find quota.cur domid)
      quota.maxent
  else Printf.sprintf "dom%i quota: not set" domid

let create () = {maxent= !maxent; maxsize= !maxsize; cur= Hashtbl.create 100}
...
let check quota id size = if !activate then _check quota id size
...
let add quota diff =
  Hashtbl.iter
    (fun id nb -> set_entry quota id (get_entry quota id + nb))
    diff.cur

let merge orig_quota mod_quota dest_quota =
  Hashtbl.iter
    (fun id nb ->
      let diff = nb - try get_entry orig_quota id with Not_found -> 0 in
      if diff <> 0 then
        set_entry dest_quota id
          ((try get_entry dest_quota id with Not_found -> 0) + diff) )
    mod_quota.cur
