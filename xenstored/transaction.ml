...
type t =
  ...
  ; start_count: int64
  ; store: Store.t
  ; (* This is the store that we change in write operations. *)
    quota: Quota.t
...
let oldest_short_running_transaction () =
  let rec last = function [] -> None | [x] -> Some x | x :: xs -> last xs in
...
let write t perm path value =
  ...
  if path_exists then set_write_lowpath t path
...
let mkdir ?(with_watch = true) t perm path =
  ...
  if with_watch then add_wop t Xenbus.Xb.Op.Mkdir path
...
let commit ~con t =
  ...
  let has_commited =
    ...
    | Full (id, oldstore, cstore) ->
        ...
        else try_commit (Store.get_root oldstore) cstore t.store
  in
  if has_commited && has_write_ops then Disk.write t.store ;
