...
let parse_config filename =
  ...
  let options =
    ...
    ; ( "xenstored-log-file"
      , Config.String Logging.set_xenstored_log_destination )
    ; ( "xenstored-log-level"
      , Config.String
...
module DB = struct
  ...
  let from_channel_f chan domain_f watch_f store_f =
    ...
    if header <> dump_format_header then raise (Bad_format "header") ;
    ...
    while not !quit do
      try
        ...
        try
          match l with
          | ["dom"; domid; mfn; port] ->
              domain_f (int_of_string domid) (Nativeint.of_string mfn)
                (int_of_string port)
          ...
          | ["store"; path; perms; value] ->
              store_f (getpath path)
                ...
                (unhexify value)
          | _ -> info "restoring: ignoring unknown line: %s" line
        with exn ->
          info "restoring: ignoring unknown line: %s (exception: %s)" line
  ...
  let from_channel store cons doms chan =
    ...
    let domain_f domid mfn port =
      let ndom =
        if domid > 0 then Domains.create doms domid mfn port
        else Domains.create0 doms
      in
  ...
  let to_file store cons file =
    ...
    in
      ...
      (fun () -> to_channel store cons channel)
...
let _ =
  ...
  in
  if cf.daemonize then Unixext.daemonize ()
  else
  ...
  Sys.set_signal Sys.sigusr1
    (Sys.Signal_handle (fun i -> sigusr1_handler store)) ;
  ...
  let process_special_fds rset =
    ...
    and do_if_set fd set fct = if List.mem fd set then fct fd in
  ...
  let periodic_ops now =
    ...
    if now > !last_stat_time +. 120. then (
      ...
      Domains.iter domains
        (Domain.log_and_reset_conflict_stats
           (info "Dom%d caused %Ld conflicts")) ;
  ...
  let main_loop () =
    ...
    List.iter
      (fun c ->
        match Connection.get_domain c with
        | None -> ()
        | Some d -> Domain.incr_io_credit d )
      peaceful_mw ;
    let start_time = Unix.gettimeofday () in
    let timeout =
      let until_next_activity =
        if Domains.all_at_max_credit domains then period_ops_interval
        else
    ...
    let rset, wset, _ =
      try Select.select (spec_fds @ inset) outset [] timeout
      with Unix.Unix_error (Unix.EINTR, _, _) -> ([], [], [])
    in
    let sfds, cfds = List.partition (fun fd -> List.mem fd spec_fds) rset in
    if List.length sfds > 0 then process_special_fds sfds ;
  ...
  while not !quit do
    try main_loop () with exc ->
      error "caught exception %s" (Printexc.to_string exc) ;
      if cf.reraise_top_level then raise exc
  done ;
