...
type domains =
  ...
  ; (* Number of domains with zero or negative credit *)
        ...
        try
          ...
          if info.Xenctrl.shutdown || info.Xenctrl.dying then (
            debug "Domain %u died (dying=%b, shutdown %b -- code %d)" id
              info.Xenctrl.dying info.Xenctrl.shutdown
              info.Xenctrl.shutdown_code ;
            if info.Xenctrl.dying then dead_dom := id :: !dead_dom
            else notify := true )
...
let create0 doms =
  let port, interface =
    ...
    and fd = Unix.openfile !xenstored_kva [Unix.O_RDWR] 0o600 in
    let interface =
      Xenmmap.mmap fd Xenmmap.RDWR Xenmmap.SHARED (Xenmmap.getpagesize ()) 0
    in
    Unix.close fd ; (port, interface)
  in
...
let decr_conflict_credit doms dom =
  ...
  if !Define.conflict_rate_limit_is_aggregate then
    ...
    else ( (* The queues are correct already: no further action needed. *) )
  else (
...
let incr_conflict_credit_from_queue doms =
  let process_queue q requeue_test =
    ...
    if requeue_test d.Domain.conflict_credit then push d q
    (* Make it queue up again for its next point of credit. *)
  in
  ...
  if !Define.conflict_rate_limit_is_aggregate then
    ...
    let inc dom =
      ...
      debug "incr_conflict_credit dom%d: %F -> %F" (Domain.get_id dom) before
        after ;
      if before <= 0.0 && after > 0.0 then doms.n_paused <- doms.n_paused - 1 ;
      if
        before < !Define.conflict_burst_limit
        && after >= !Define.conflict_burst_limit
